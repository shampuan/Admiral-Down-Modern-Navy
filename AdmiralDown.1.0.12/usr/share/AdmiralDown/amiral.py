...
class DenizIzgarası(QWidget):
    ...
    def verileri_sifirla(self):
        ...
        # Sadece oyuncunun kendi tahtası klavye odağı alabilsin (R/Z tuşları); düşman tahtası hiç odaklanmasın
        self.setFocusPolicy(Qt.StrongFocus if self.tip == "oyuncu" else Qt.NoFocus)
    ...
    def __init__(self, tip="oyuncu"):
        ...
        self.hucre_filmleri = {} # (x,y): QMovie -- her hücrenin kendi patlama/sıçrama animasyonu
    ...
    def paintEvent(self, event):
        ...
        for (vx, vy), veri in list(self.vuruslar.items()):
            ...
            if not anim_bitti:
                # Her hücrenin kendi animasyon nesnesi var, ortak film kullanılmıyor
                movie = self.hucre_filmleri.get((vx, vy))
        ...
        if not self.yerlestirme_modu:
            if self.tip == "dusman" and event.button() == Qt.LeftButton:
                if 0 <= x < 10 and 0 <= y < 10:
                    ana = self.window()
                    # Oyun zaten bittiyse (son gemi vurulduysa) artık ateş edilemez
                    if ana.oyun_gercekten_bitti:
                        return
                    ...
                    if "PLAYER" in ana.durum_label.text() and (x, y) not in self.vuruslar:
                        ...
                        if not isabetli:
                            ...
                            QTimer.singleShot(300, ana.yz_ates_et)
    ...
    def enterEvent(self, event):
        """Fare denizin üzerine geldiğinde klavye odağını buraya al (sadece oyuncu tahtasında)"""
        if self.tip == "oyuncu":
            self.setFocus()
    ...
    def keyPressEvent(self, event):
        ...
        if self.yerlestirme_modu:
            ...
            # Z TUŞU: SON YERLEŞTİRİLEN GEMİYİ GERİ ALIR (sadece oyuncunun kendi tahtasında çalışmalı)
            elif event.key() == Qt.Key_Z and self.tip == "oyuncu":
    ...
    def vurus_yap(self, x, y):
        ...
        if isabet:
            ...
            # Her isabet için kendi patlama animasyonu (ortak nesne kullanılmıyor, ard arda atışlar birbirini kesmesin diye)
            hucre_movie = QMovie(self.explosion_movie.fileName())
        else:
            ...
            if self.ocean_movie.fileName() == tehlike_yolu:
                aktif_splash_yolu = self.splash_red_movie.fileName()
            else:
                aktif_splash_yolu = self.splash_movie.fileName()
            
            # Her ıskalama için kendi sıçrama animasyonu
            hucre_movie = QMovie(aktif_splash_yolu)
        
        if hucre_movie.isValid():
            hucre_movie.setCacheMode(QMovie.CacheAll)
            hucre_movie.frameChanged.connect(self.update)
            hucre_movie.start()
        self.hucre_filmleri[(x, y)] = hucre_movie
...
class AnaPencere(QMainWindow):
    ...
    def portre_guncelle(self, flas_yap=True):
        ...
        # 4. Görselleri Uygula ve Flaş Çaktır (art arda fark edilsin diye aralarına gecikme koyduk)
        self._portre_set_ve_flash(self.lbl_oyuncu_resim, oyuncu_yeni_resim, flas_yap)
        if flas_yap:
            QTimer.singleShot(250, lambda: self._portre_set_ve_flash(self.lbl_yz_resim, yz_yeni_resim, flas_yap))
        else:
            self._portre_set_ve_flash(self.lbl_yz_resim, yz_yeni_resim, flas_yap)

    def _portre_set_ve_flash(self, label, dosya_adi, flas_yap=True):
    ...
    def resizeEvent(self, event):
        # Pencere boyutu her değiştiğinde (Tam ekran dahil) portreleri yeniden boyutlandır (flaş yok, sadece boyut)
        self.portre_guncelle(flas_yap=False)
        super().resizeEvent(event)

    def skor_guncelle(self):
        # Düşman donanmasındaki batık/kalan gemi sayısına göre skor alanını tazeler
        toplam_gemi = len(self.dusman_denizi.donanma_yapisi)
        batan_sayisi = sum(1 for gemi in self.dusman_denizi.gemiler if gemi.get('batık'))
        self.skor_label.setText(f"Sunk: {batan_sayisi}\nRemaining: {toplam_gemi - batan_sayisi}")
    ...
    def yz_aksiyonu_tamamla(self, x, y):
        ...
        if isabetli:
            ...
            QTimer.singleShot(300, self.yz_ates_et)
        else:
